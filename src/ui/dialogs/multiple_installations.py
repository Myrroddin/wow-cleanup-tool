...
class MultipleInstallationsDialog(BaseDialog):
    ...
    def __init__(self, parent, loc, theme_name, settings, installations):
        ...
        """
        super().__init__(parent, loc, theme_name, settings, 'title_multiple_installations')
    ...
    def show(self):
        ...
        installations_frame = ttk.LabelFrame(
            content,
            text=self.loc._("label_detected_installations"),
            padding=10
        )
        ...
        ok_button = self.create_button(button_frame, 'ok', command=lambda: dialog.destroy())
...
def show_multiple_installations_dialog(parent, loc, theme_name, settings, installations):
    ...
    Args:
        ...
        installations: List of detected installation paths
    """
    dialog = MultipleInstallationsDialog(parent, loc, theme_name, settings, installations)
    dialog.show()
