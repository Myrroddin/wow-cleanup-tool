...
class LicenseDialog(BaseDialog):
    ...
    def show(self):
        ...
        instructions = self.create_message_label(
            content, 'msg_license_instructions',
            wraplength=DialogDimensions.WRAP_LONG
        )
        instructions.pack(pady=(0, DialogDimensions.SPACING_MEDIUM))
        
        # License text frame with scrollbar
        text_frame = ttk.Frame(content)
        text_frame.pack(fill='both', expand=True, pady=(0, DialogDimensions.SPACING_LARGE))
        ...
        license_text = tk.Text(text_frame, wrap='none', 
                              yscrollcommand=scrollbar.set,
                              width=optimal_width, height=25,
                              bg=theme['entry_bg'],
                              fg=theme['entry_fg'],
        ...
        dont_show, dont_show_var = self.create_checkbox(content, 'option_license_dont_show')
        ...
        decline_button = self.create_button(
            button_frame, 'license_decline',
            command=lambda: self._on_decline(dialog)
        )
...
def show_license_dialog(parent, loc, theme_name, settings):
    ...
    if settings.get('license_accepted', False) and settings.get('disable_license_dialog', False):
        return True
