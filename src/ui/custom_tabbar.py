...
class CustomTabBar(tk.Frame):
    def __init__(
        self, parent, tabs, on_tab_selected, theme_colors=None, *args, **kwargs
    ):
        ...
        style.map(
            "Tab.TButton",
            background=[
                ("pressed", self.theme_colors["select_bg"]),
                ("active", self.theme_colors["frame_bg"]),
                ("!active", self.theme_colors["button_bg"]),
            ],
            foreground=[
                ("pressed", self.theme_colors["select_fg"]),
    ...
    def _build(self):
        ...
        for idx, (tab_id, label) in enumerate(self.tabs):
            ...
            btn.place(
                in_=self, relx=0, rely=0, x=idx * 160, y=0
            )  # Use place to keep above bg_frame
