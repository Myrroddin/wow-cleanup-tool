...
class WoWLogger:
    ...
    def __init__(self, verbose: bool = False, append_mode: bool = False):
        ...
        # Create formatters
        self.user_formatter = logging.Formatter('[%(asctime)s] %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
        self.dev_formatter = logging.Formatter('[%(asctime)s] [%(levelname)s] %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
