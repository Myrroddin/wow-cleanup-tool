...
def show_startup_warning(root, settings, save_settings_fn, logger=None):
    ...
    """
    loc = localization.Localization(settings.get("language", localization.DEFAULT_LANGUAGE))
    ...
    popup.title(loc._("important_notice"))
    ...
    ttk.Label(
        frm,
        text=loc._("startup_warning_text"),
        wraplength=pw - 24, justify="left"
    ).pack(pady=(0, 12))

    never_show_var = tk.BooleanVar(value=False)
    ttk.Checkbutton(frm, text=loc._("do_not_show_again"), variable=never_show_var).pack(anchor="w")
    ...
    def _dismiss():
        if never_show_var.get():
            try:
                ...
                if logger:
                    try: logger.log(loc._("user_disabled_warning"))
                    except Exception: pass
            except Exception:
                pass
        popup.destroy()

    ttk.Button(frm, text=loc._("ok"), command=_dismiss).pack(pady=(12, 0))
