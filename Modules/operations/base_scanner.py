...
class BaseScanner:
    ...
    def __init__(self, max_workers: int = 8, logger: Optional[Any] = None, loc: Optional[Any] = None):
        """Initialize scanner.
    ...
    def scan_versions(
        ...
        try:
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        if self.logger and hasattr(self, 'loc') and self.loc:
                            self.logger.error(self.loc._("error_scanning_version").format(vlabel, e))
                        elif self.logger:
            ...
            if self.logger and hasattr(self, 'loc') and self.loc:
                ...
                self.logger.error(f"ThreadPoolExecutor failed, falling back to single-threaded: {e}")
            for vpath, vlabel in versions:
                try:
