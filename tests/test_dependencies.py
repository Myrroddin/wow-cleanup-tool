...
Updated: January 3, 2026 - Removed beta/alpha fallback, stable releases only
"""
...
class TestDependencyManager(unittest.TestCase):
    ...
    def test_install_missing_dependencies_parallel_execution(self):
        """Test install_missing_dependencies uses ThreadPoolExecutor with 2 workers max."""
        self.manager.missing_packages = ["send2trash", "PIL"]
        ...
        with patch("core.dependencies.ThreadPoolExecutor") as mock_executor_class:
            ...
            with patch(
                ...
                return_value=[mock_future1, mock_future1],
            ):
                with patch.object(self.manager, "_install_package") as mock_install:
                    ...
                    # max_workers should be min(3, 2) = 2
                    max_workers_arg = call_kwargs.get("max_workers") or (
                        call_args[0] if call_args else 2
                    )
                    assert max_workers_arg == 2
    ...
    def test_install_package_stable_success(self):
        ...
        with patch("subprocess.check_call") as mock_check_call:
            ...
            assert "send2trash>=2.0.0" in call_args

    def test_install_package_installation_fails(self):
        """Test _install_package returns False when installation fails."""
        with patch("subprocess.check_call") as mock_check_call:
            import subprocess
    ...
    def test_install_package_30_second_timeout(self):
        ...
        with patch("subprocess.check_call") as mock_check_call:
            self.manager._install_package("send2trash>=2.0.0")
    ...
    def test_install_package_progress_callback(self):
        """Test _install_package calls progress_callback during installation."""
        progress_callback = MagicMock()

        with patch("subprocess.check_call"):
            self.manager._install_package(
                "send2trash>=2.0.0", progress_callback=progress_callback
            )

            # Should call progress_callback once with package name
            assert progress_callback.called
            progress_callback.assert_called_once_with("send2trash")

    def test_install_package_progress_to_queue(self):
        """Test _install_package sends progress messages to queue when callback provided."""
        update_queue = queue.Queue()
        progress_callback = MagicMock()

        with patch("subprocess.check_call"):
            # Method should accept update_queue with progress_callback
            success, message, timed_out = self.manager._install_package(
                "send2trash>=2.0.0",
            ...
            # Verify progress message was queued
            assert not update_queue.empty()
            msg = update_queue.get()
            assert msg == ("progress", "send2trash")
    ...
    def test_mixed_success_and_failure(self):
        ...
        with patch.object(self.manager, "_install_package", side_effect=mock_install):
            ...
            # Overall success should be False due to PIL installation failure
            assert result is False

    def test_package_name_extraction(self):
        """Test package name is extracted correctly from spec."""
        with patch("subprocess.check_call"):
            self.manager._install_package("Pillow>=12.1.0")
